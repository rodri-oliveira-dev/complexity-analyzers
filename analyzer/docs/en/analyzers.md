...
This page is the public catalog of diagnostics exposed by `ComplexityAnalysis.Analyzers` in Phase 5.

The analyzer resolves known BCL and LINQ operations through Roslyn symbols, and can propagate complexity from safe source methods in the same compilation. Same-name custom methods are not treated as known BCL/LINQ operations. Unsupported, unsafe, cyclic, budget-limited, or unresolved operations remain `Unknown`.
...
`BIG0001` is informational. It exposes the analyzer's known estimate for a supported method, such as `O(1)`, `O(n)`, `O(n log n)`, or `O(n^2)`. In Phase 5, that estimate can include complexity propagated from safe source-method callees.
...
- the method depends on unsupported, unsafe, cyclic, budget-limited, or unresolved operations.
...
Phase 5 includes a small documented subset:
